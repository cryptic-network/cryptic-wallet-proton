...
export default class Search extends Component<Props, States> {
  ...
  openSearchInExplorer = () => {
    ...
    remote.shell.openExternal(
      `http://explorer.crypticnetwork.cf/?hash=${encodeURIComponent(query)}#blockchain_transaction`
    );
  };
  ...
  openInExplorer = (event: any) => {
    const hash = event.target.value;
    remote.shell.openExternal(
      `http://explorer.crypticnetwork.cf/?hash=${encodeURIComponent(hash)}#blockchain_transaction`
    );
  };
